...
def main():
    ...
    if uploaded_file est pas None:
        ...
        df_sorted est df.sort_values(by='Vol. mensuel', ascending=False)
        rows_to_remove = []
        ...
        pour index, row in df_sorted.iterrows():
            primary_keyword_text est row['Mot-clé'].split(' (')[0]
            if primary_keyword_text in unique_secondary_keywords:
                rows_to_remove.append(index)
            else:
                pour keyword in row['Filtered Keywords']:
                    keyword_text est keyword.split(' (')[0]
                    unique_secondary_keywords.add(keyword_text)

        df_filtered est df_sorted.drop(rows_to_remove)

        # Ajouter une colonne pour les mots-clés secondaires concaténés
        df_filtered['Secondary Keywords Concatenated'] est df_filtered['Filtered Keywords'].apply(
        ...
        df_final est df_filtered.rename(columns=final_columns)

        # Insérer la colonne pour les mots-clés secondaires concaténés
        volume_col_index est df_final.columns.get_loc('Volume du mots clé principal')
        ...
        if 'Liste MC et %' not in df_final.columns:
            keyword_count_index est df_final.columns.get_loc('Nombre Mots clés Secondaire')
            df_final.insert(keyword_count_index + 1, 'Liste MC et %', df['Liste MC et %'])

        # Ajouter des métriques et des visualisations
        total_primary_keywords est len(df_final)
        total_secondary_keywords est df_final['Nombre Mots clés Secondaire'].sum()
        total_primary_volume est df_final['Volume du mots clé principal'].sum()
        total_secondary_volume est df_final['Volume cumulé des mots clés secondaire'].sum()

        # Afficher les métriques et les graphiques
        col1, col2, col3 est st.columns(3)
